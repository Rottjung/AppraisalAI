...
[Serializable]
public sealed class BehaviorNode : NodeBase
{
    ...
    public BehaviorNode(string id, string displayName) : base(id, displayName) { }
    ...
    public Connection GetConnection(string sourceNodeId)
    {
        for (int i = 0; i < connections.Count; i++)
        {
            Connection c = connections[i];
            if (c != null && c.SourceNodeId == sourceNodeId)
            {
                return c;
            }
        }
